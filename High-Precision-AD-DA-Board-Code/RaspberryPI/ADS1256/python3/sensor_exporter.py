...
class DLC_sensor_Collector(object):
    ...
    def chassis_stabil_calc(self):
        ...
        print("boot value: ", init_x)
        print("current value: ",curr_x)
        print("offset value: ", abs(init_x-curr_x))
        is_stable = 1

        # Compare current xyz coordinate data with boot xyz data. 
        if abs(curr_x - init_x) > 2 or abs(curr_y - init_y) > 3 or abs(curr_z - init_z) > 3:
            is_stable = 0
        else:
            is_stable = 1
        return is_stable

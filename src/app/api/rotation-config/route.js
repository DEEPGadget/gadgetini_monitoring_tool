...
export async function POST(request) {
  try {
    ...
    for (const node of rows) {
      await updateRotation(node.username, node.ipaddress, node.password, rotation);
    }

    return new Response(JSON.stringify({ message: "Rotation updated on all nodes!" }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
    console.error("Error updating rotation:", error);
    return new Response(JSON.stringify({ error: "Failed to update rotation." }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    });
  }
